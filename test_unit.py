...
class TestTermsAcceptanceEndpoints(unittest.TestCase):
    """Test /api/terms POST and GET endpoints."""

    def setUp(self):
        from merkraum_api import app
        os.environ["AUTH_REQUIRED"] = "false"
        app.config["TESTING"] = True
        self.app = app
        self.client = app.test_client()

        # Mock adapter with a Neo4j driver mock
        mock_adapter = MagicMock()
        mock_session = MagicMock()
        mock_adapter._driver.session.return_value.__enter__ = MagicMock(return_value=mock_session)
        mock_adapter._driver.session.return_value.__exit__ = MagicMock(return_value=False)
        self._mock_session = mock_session

        import merkraum_api
        self._original_adapter = merkraum_api.adapter
        merkraum_api.adapter = mock_adapter

    def tearDown(self):
        import merkraum_api
        merkraum_api.adapter = self._original_adapter

    def test_post_terms_requires_json(self):
        resp = self.client.post("/api/terms", data="not json")
        self.assertEqual(resp.status_code, 400)

    def test_post_terms_requires_version(self):
        resp = self.client.post("/api/terms", json={"terms_accepted_at": "2026-03-16T12:00:00Z"})
        self.assertEqual(resp.status_code, 400)
        self.assertIn("terms_version", resp.get_json()["error"])

    def test_post_terms_requires_accepted_at(self):
        resp = self.client.post("/api/terms", json={"terms_version": "v1"})
        self.assertEqual(resp.status_code, 400)
        self.assertIn("terms_accepted_at", resp.get_json()["error"])

    def test_post_terms_success(self):
        # AUTH_REQUIRED=false → dev-user fallback used
        resp = self.client.post("/api/terms", json={
        ...
        self.assertEqual(data["terms_version"], "2026-03-16-v1")

    def test_get_terms_not_found(self):
        self._mock_session.run.return_value.single.return_value = None
        resp = self.client.get("/api/terms")
        self.assertEqual(resp.status_code, 404)

    def test_get_terms_found(self):
        self._mock_session.run.return_value.single.return_value = {
            "user_id": "dev-user",
            "terms_version": "2026-03-16-v1",
            "terms_accepted_at": "2026-03-16T12:00:00.000Z",
            "recorded_at": "2026-03-16T12:00:01.000Z",
        }
        resp = self.client.get("/api/terms")
